...
class FIREFOX(Enum):
    ...
    HORIZONTAL = 2
    RANDOM = 3
    # Added SAFERANDOM option so custombot wouldn't random a straight horizontal upB and SD below the stage
    SAFERANDOM = 4
...
class Firefox(Chain):
    ...
    def getangle(self, gamestate, custombot_state):
        ...
        diff_y = abs(custombot_state.position.y + 5)
        ...
        if custombot_state.position.y < -10:
            wanted_angle = math.degrees(-math.atan2(x - 0.5, y - 0.5)) % 360
            lowest_angle = math.degrees(-math.atan2(corner_x - custombot_state.position.x, corner_y - custombot_state.position.y)) % 360
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        # We're done here if...
        if custombot_state.on_ground or custombot_state.action in [Action.EDGE_CATCHING, Action.EDGE_HANGING, Action.SWORD_DANCE_1_AIR]:
            self.interruptible = True
            controller.empty_input()
            return

        # If we're traveling in the air, let go of the stick
        if custombot_state.action in [Action.FIREFOX_AIR, Action.DEAD_FALL]:
            self.interruptible = False
            controller.empty_input()
            return

        # We need to jump out of our shine
        if custombot_state.action in [Action.DOWN_B_AIR, Action.DOWN_B_STUN]:
        ...
        if custombot_state.action == Action.FIREFOX_WAIT_AIR:
            self.interruptible = False
            if self.direction == FIREFOX.HIGH:
                if diff_x > 20:
                    ...
                    return
            if self.direction == FIREFOX.HORIZONTAL and custombot_state.position.y > -8:
                controller.tilt_analog(Button.BUTTON_MAIN, x, .5)
                return
            if self.direction == FIREFOX.EDGE:
                x, y = self.getangle(gamestate, custombot_state)
        ...
        if self.direction == FIREFOX.EDGE:
            ...
            if (.3525 < y < .6475) or (.3525 < x < .6475) and (custombot_state.position.y > -15):
                # Unless we're in range to just grab the edge. Then the angle doesn't matter
                if not ((-16.4 < custombot_state.position.y < -5) and (diff_x < 10)):
                    controller.empty_input()
                    return
