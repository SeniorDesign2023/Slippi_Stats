...
class AirAttack(Chain):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if custombot_state.action in [Action.LANDING, Action.UAIR_LANDING, Action.FALLING]:
            self.interruptible = True
            controller.release_all()
            return

        # We've somehow fallen off stage
        if custombot_state.position.y < 0:
            self.interruptible = True
            controller.release_all()
            return
        ...
        if custombot_state.on_ground:
            ...
            if controller.prev.button[Button.BUTTON_Y] and custombot_state.action != Action.KNEE_BEND:
                ...
                x = int(self.target_x > custombot_state.position.x)
                if abs(self.target_x - custombot_state.position.x) < 10:
                    x = 0.5
        ...
        if custombot_state.speed_y_self < 0:
            ...
            if abs(custombot_state.position.x) + 10 > melee.stages.EDGE_GROUND_POSITION[gamestate.stage]:
                controller.tilt_analog(Button.BUTTON_MAIN, int(custombot_state.position.x < 0), 0)
                return
            else:
                # fastfall
        ...
        if self.height_level == 2:
            # Up-air on frame 11
            if custombot_state.action in [Action.JUMPING_FORWARD, Action.JUMPING_BACKWARD] and custombot_state.action_frame >= 11:
                controller.tilt_analog(Button.BUTTON_C, 0.5, 1)
                return
            if custombot_state.action in [Action.UAIR, Action.BAIR, Action.DAIR, Action.FAIR, Action.NAIR]:
                # Fast fall on frame 8
                if custombot_state.action_frame >= 8:
                    controller.tilt_analog(Button.BUTTON_MAIN, 0.5, 0)
                    return

            controller.tilt_analog(Button.BUTTON_MAIN, int(self.target_x > custombot_state.position.x), .5)
            controller.tilt_analog(Button.BUTTON_C, 0.5, 0.5)
            return

        # Okay, we're jumping in the air, now what?
        if custombot_state.action in [Action.JUMPING_FORWARD, Action.JUMPING_BACKWARD, Action.JUMPING_ARIAL_FORWARD, Action.JUMPING_ARIAL_BACKWARD]:
            ...
            if custombot_state.action in [Action.JUMPING_FORWARD, Action.JUMPING_BACKWARD]:
                if custombot_state.action_frame == jump_on_frame:
                    controller.press_button(Button.BUTTON_Y)
                    controller.tilt_analog(Button.BUTTON_MAIN, int(self.target_x > custombot_state.position.x), .5)
                    return
                else:
            ...
            if custombot_state.action in [Action.JUMPING_ARIAL_FORWARD, Action.JUMPING_ARIAL_BACKWARD]:
