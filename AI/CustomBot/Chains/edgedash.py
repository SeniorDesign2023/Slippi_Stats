...
class Edgedash(Chain):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if self.logger:
            self.logger.log("Notes", " Distance to edge : " + str(custombot_state.position.y + custombot_state.ecb.bottom.y) + " ", concat=True)
        ...
        if custombot_state.action == Action.SWORD_DANCE_3_LOW:
        ...
        if not self.hasstalled and self.refresh:
            ...
            if custombot_state.action == Action.EDGE_HANGING:
                ...
                if controller.prev.c_stick != (0.5, 0.5):
                    ...
                    return
                self.interruptible = False
                controller.tilt_analog(Button.BUTTON_C, int(custombot_state.position.x > 0), 0.5)
                return
        ...
        if custombot_state.action == Action.FALLING:
            self.interruptible = False
            controller.tilt_analog(Button.BUTTON_MAIN, int(custombot_state.position.x < 0), 0.5)
        ...
        if custombot_state.action == Action.JUMPING_ARIAL_FORWARD:
            ...
            if custombot_state.position.y + custombot_state.ecb.bottom.y > 0.25:
                airdodge_angle = 0.2
                # Go at a more horizontal angle if we're only just barely above the stage
                if custombot_state.position.y + custombot_state.ecb.bottom.y < 1:
                    airdodge_angle = 0.35

                self.interruptible = False
                controller.tilt_analog(Button.BUTTON_MAIN, int(custombot_state.position.x < 0), airdodge_angle)
