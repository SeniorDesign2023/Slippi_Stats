...
""" Also Called Perfect Shield in the wiki."""
class Powershield(Chain):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if attackstate == melee.AttackState.ATTACKING and custombot_state.action in [Action.SHIELD_REFLECT, Action.SHIELD]:
            self.interruptible = False
            controller.tilt_analog(Button.BUTTON_MAIN, 0.5, 0.5)
            controller.press_button(Button.BUTTON_L)
            return
        ...
        if custombot_state.action == Action.SHIELD_RELEASE:
            self.interruptible = True
            controller.empty_input()
            return

        isshielding = custombot_state.action == Action.SHIELD \
            or custombot_state.action == Action.SHIELD_START \
            or custombot_state.action == Action.SHIELD_REFLECT \
            or custombot_state.action == Action.SHIELD_STUN \
            or custombot_state.action == Action.SHIELD_RELEASE

        # If we're in shield stun, we can let go
        if custombot_state.action == Action.SHIELD_STUN:
            if custombot_state.hitlag_left > 0:
                self.interruptible = False
                ...
                if controller.prev.main_stick[0] == 0.5:
                    if self.direction is None:
                        ...
                        di_distance = 3.96 * (custombot_state.hitlag_left // 2)
                        shine_range = 11.8
                        shield_slide = 10
                        sdi_in = abs(opponent_state.position.x - custombot_state.position.x) < di_distance + shine_range - shield_slide
                        if opponent_state.off_stage:
                            sdi_in = False
                        if sdi_in:
                            self.direction = int(opponent_state.position.x > custombot_state.position.x)
                        else:
                            self.direction = int(opponent_state.position.x < custombot_state.position.x)
