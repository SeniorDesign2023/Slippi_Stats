...
class SDI(Chain):
    ...
    def touching_ground(custombot_state):
        ...
        if abs(custombot_state.position.y) < 0.25:
            return True
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if opponent_state.character == Character.SHEIK and opponent_state.action == Action.SWORD_DANCE_2_HIGH:
            if gamestate.frame % 2:
                controller.tilt_analog(Button.BUTTON_MAIN, int(opponent_state.position.x > custombot_state.position.x), 0.5)
            else:
        ...
        if self.cardinal is not None:
            ...
            if SDI.touching_ground(custombot_state):
        ...
        if opponent_state.character in [Character.PEACH, Character.PIKACHU, Character.SAMUS, Character.SHEIK] and opponent_state.action == Action.DOWNSMASH:
            angle = math.degrees(-math.atan2(custombot_state.speed_x_attack, custombot_state.speed_y_attack)) + 90
        ...
        if custombot_state.off_stage:
            cardinal = (int(custombot_state.position.x < 0), int(custombot_state.position.y < 0))
            if self.logger:
        ...
        absolute_speed = math.sqrt(custombot_state.speed_x_attack ** 2 + custombot_state.speed_y_attack ** 2)

        # Survival SDI
        #   If we're at risk of dying from the hit, then SDI backwards to go further back to cut into the knockback
        if custombot_state.percent > 60 and absolute_speed > 3:
            # Amsah tech
            #   If we're in survival DI, and near the ground, then let's Amsah tech
            if custombot_state.position.y < 6:
                self.cardinal = (int(custombot_state.position.x < 0), 0)
            else:
                angle = math.degrees(-math.atan2(custombot_state.speed_x_attack, custombot_state.speed_y_attack)) + 90
                ...
                self.cardinal = SDI.angle_to_cardinal(angle)
                if self.logger:
                    self.logger.log("Notes", " Survival SDI angle: " + str(angle) + " " + str(custombot_state.speed_y_attack) + " " + str(custombot_state.speed_x_attack), concat=True)
        ...
        angle = math.degrees(-math.atan2(custombot_state.position.x - opponent_state.position.x, custombot_state.position.y - opponent_state.position.y)) + 90
        angle = (angle + 360) % 360
        self.cardinal = SDI.angle_to_cardinal(angle)
        if self.logger:
