...
class TDI(Chain):
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if custombot_state.percent > 60 and absolute_speed > 3:
            ...
            angle = (math.degrees(-math.atan2(custombot_state.speed_x_attack, custombot_state.speed_y_attack)) + 90) % 360
        ...
        if SDI.touching_ground(custombot_state):
            ...
            if abs(melee.stages.EDGE_POSITION[gamestate.stage] - abs(custombot_state.position.x)) > 30:
                if gamestate.custom["tech_lockout"] == 0:
                    controller.press_button(Button.BUTTON_L)
                else:
                    controller.release_button(Button.BUTTON_L)
