...
class Approach(Tactic):
    ...
    def shouldapproach(custombot_state, opponent_state, gamestate, framedata, logger):
    ...
    def approach_too_dangerous(custombot_state, opponent_state, gamestate, framedata):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        losing = custombot_state.stock < opponent_state.stock or (custombot_state.stock == opponent_state.stock and custombot_state.percent > opponent_state.percent)
        ...
        """ Should be adjustable based on difficulty """
        ...
        if self.random_approach < 33:
            if not self.framedata.is_attack(opponent_state.character, opponent_state.action):
                ...
                vertical_distance = abs(custombot_state.position.y - opponent_state.position.y)
                facing_opponent = custombot_state.facing == (custombot_state.position.x < opponent_state.position.x)
                if custombot_state.action == Action.DASHING and facing_opponent:
                    if vertical_distance < 20 and gamestate.distance < 35 and abs(melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - abs(custombot_state.position.x)) > 35:
        ...
        in_position = abs(custombot_state.position.x - pivotpoint) < 5
