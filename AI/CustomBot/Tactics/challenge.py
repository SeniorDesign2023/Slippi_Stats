...
class Challenge(Tactic):
    ...
    def canchallenge(custombot_state, opponent_state, gamestate, framedata, difficulty):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if opponent_state.character == Character.DK:
            bufferzone = 40
            if opponent_state.facing != (opponent_state.position.x < custombot_state.position.x):
                bufferzone = 40
        ...
        if on_side_plat and abs(custombot_state.position.x - pivotpoint) < 2 and custombot_state.action == Action.TURNING:
        ...
        if abs(custombot_state.position.x - pivotpoint) < spacing_grace_zone:
            if custombot_state.action == Action.TURNING:
                if smash_now and not on_side_plat and not falling_spacie:
                    ...
                    return
                if falling_spacie and abs(opponent_state.position.y - custombot_state.position.y) < 36:
                    self.chain = None
                    self.pickchain(Chains.Tilt, [TILT_DIRECTION.UP])
                    return
            elif custombot_state.action == Action.DASHING:
                # Ground pound
                if opponent_state.character == Character.DK and opponent_state.action in [Action.DK_GROUND_POUND]:
                    if custombot_state.facing == (custombot_state.position.x < opponent_state.position.x):
