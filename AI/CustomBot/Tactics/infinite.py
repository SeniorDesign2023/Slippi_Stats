...
""" Probably just remove this class entirely, to prevent any accidental bot's from spamming infinites
    Could set a limit on this so it can only be performed a certain number of times instead of infinitely """
class Infinite(Tactic):
    ...
    def caninfinite(custombot_state, opponent_state, gamestate, framedata, difficulty):
        ...
        if custombot_state.position.y > 2 or opponent_state.position.y > 2:
            return False
        ...
        framesleft = Punish.framesleft(opponent_state, framedata, custombot_state)
        ...
        if (custombot_state.action == Action.DASHING and custombot_state.action_frame >= 11):
            if (custombot_state.speed_ground_x_self > 0) == (custombot_state.position.x > 0):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        distance_from_edge = melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - abs(custombot_state.position.x)
        edgetooclose = custombot_state.action == Action.EDGE_TEETERING_START or distance_from_edge < 5
        ...
        if gamestate.distance < shinerange and not edgetooclose:
            ...
            if custombot_state.action in [Action.RUNNING, Action.RUN_BRAKE, Action.CROUCH_START] and distance_from_edge < 16:
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
            ...
            if (custombot_state.position.x < opponent_state.position.x < 0) or (0 < opponent_state.position.x < custombot_state.position.x):
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
            ...
            if (custombot_state.speed_ground_x_self > 0) == onright:
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
        ...
        if not (custombot_state.action == Action.DOWN_B_GROUND_START and custombot_state.action_frame in [1,2]):
            self.pickchain(Chains.Run, [opponent_state.position.x > custombot_state.position.x])
            return
        return
