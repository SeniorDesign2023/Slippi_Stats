...
class KeepDistance(Tactic):
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if opponent_state.position.x < custombot_state.position.x < 0 or \
                0 < custombot_state.position.x < opponent_state.position.x:
            edgebuffer = 10
        ...
        if custombot_state.action == Action.TURNING and (opponent_state.position.x < custombot_state.position.x) == custombot_state.facing:
            if (random.randint(0, 30) == 0):
                self.stand_menacingly = True

        if self.framedata.is_attack(opponent_state.character, opponent_state.action) or abs(pivotpoint - custombot_state.position.x) > 10:
                self.stand_menacingly = False
        ...
        """ This needs to be changed from Firefox to be just up-b so it works with most characters """
        if not custombot_state.off_stage:
            self.pickchain(Chains.DashDance, [pivotpoint, self.radius])
        # If for whatever reason keepdistance gets called while custombot is recovering, it will do an emergency Firefox"""
        else:
