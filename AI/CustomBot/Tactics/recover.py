...
class Recover(Tactic):
    ...
    def needsrecovery(custombot_state, opponent_state, gamestate):
        ...
        # If the opponent is on-stage, and custombot is on-edge, custombot needs to ledgedash
        ...
        custombot_dist = math.sqrt( (custombot_state.position.y)**2 + (diff_x)**2 )

        if opponent_dist < custombot_dist and not onedge:
            return True

        if custombot_dist >= 20:
            return True

        # If we're both fully off stage, recover
        if opponent_state.off_stage and custombot_state.off_stage and not onedge and not opponentonedge:
            return True
    ...
    """ Set up for only Fox/Falco Firefox and Illusion, so will have to add for each other character """
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        if custombot_state.action in [Action.EDGE_HANGING, Action.EDGE_CATCHING]:
            ...
            frames_left = Punish.framesleft(opponent_state, self.framedata, custombot_state)
            refresh = frames_left < 16 and custombot_state.invulnerability_left < 16
            self.pickchain(Chains.Edgedash, [refresh])
            return

        # If we can't possibly illusion to recover, don't try
        if custombot_state.position.y < -15 and custombot_state.jumps_left == 0 and custombot_state.speed_y_self < 0:
        ...
        if not facinginwards and custombot_state.action == Action.TURNING and custombot_state.action_frame == 1:
            facinginwards = True

        if custombot_state.action == Action.DEAD_FALL:
            self.chain = None
            self.pickchain(Chains.DI, [int(custombot_state.position.x < 0), 0.5])
            return

        # FireFox high
        if custombot_state.action == Action.SWORD_DANCE_1_AIR and custombot_state.position.y > 10:
            self.chain = None
            self.pickchain(Chains.DI, [int(custombot_state.position.x < 0), 0.5])
            return
        ...
        if -12 < custombot_state.position.y and (diff_x < 10) and (facinginwards or custombot_state.action == Action.SWORD_DANCE_1_AIR) and custombot_state.speed_y_self <= 0:
            # Do a Fastfall if we're not already
            if custombot_state.action == Action.FALLING and custombot_state.speed_y_self > -3.3:
                self.chain = None
                self.pickchain(Chains.DI, [0.5, 0])
                return

            # If we are currently moving away from the stage, DI in
            if (custombot_state.speed_air_x_self > 0) == (custombot_state.position.x > 0):
                x = 0
                if custombot_state.position.x < 0:
                    x = 1
                self.chain = None
                ...
                if custombot_state.action == Action.SLIDING_OFF_EDGE and gamestate.stage == Stage.BATTLEFIELD:
                    self.chain = None
                    x = 0
                    if custombot_state.position.x > 0:
                        x = 1
        ...
        if gamestate.stage == melee.enums.Stage.BATTLEFIELD:
            min_height = -10
        if self.useillusion and (min_height < custombot_state.position.y < -5) and (10 < diff_x < 88) and not opponentonedge:
            length = SHORTEN.LONG
            if diff_x < 50:
                length = SHORTEN.MID
            if diff_x < 40:
        ...
        # If we illusion at this range when the opponent is holding ledge, custombot dies.
        # Firefox instead if the opponent is grabbing edge. Opponent has to get up or get burned.
        if (-16.4 < custombot_state.position.y < -5) and (diff_x < 10) and facinginwards:
            if gamestate.stage == melee.enums.Stage.BATTLEFIELD:
                # If custombot does a random or horizontal sideB here, he pretty reliably SDs on Battlefield
        ...
        hit_movement = abs(custombot_state.speed_x_attack) > 0.2

        x_canairdodge = abs(custombot_state.position.x) - 18 <= abs(melee.stages.EDGE_GROUND_POSITION[gamestate.stage])
        y_canairdodge = custombot_state.position.y >= -24
        if x_canairdodge and y_canairdodge and (opponentgoingoffstage or opponentmovingtoedge) and not hit_movement:
            self.pickchain(Chains.Airdodge, [int(custombot_state.position.x < 0), int(custombot_state.position.y + custombot_state.ecb.bottom.y < 5)])
            return

        # Jump if we're falling, are below stage, and have a jump
        if custombot_state.speed_y_self < 0 and custombot_state.jumps_left > 0 and custombot_state.position.y < 0:
        ...
        if custombot_state.jumps_left > 0 and (custombot_state.position.y < -52 or opponentgoingoffstage or opponentmovingtoedge or jump_randomizer):
            self.pickchain(Chains.Jump)
            return
        ...
        if self.useillusion and custombot_state.position.y > -5:
            # DI into the stage
            x = 0
            if custombot_state.position.x < 0:
                x = 1
        ...
        if custombot_state.speed_y_self < 0 and custombot_state.position.y < firefox_height:
        ...
        randomhighrecovery = custombot_state.position.y > 0 and random.randint(0, 3) == 1
        ...
        # DI into the stage
        battlefielded = (custombot_state.position.x < melee.stages.EDGE_POSITION[gamestate.stage] + 13) and gamestate.stage == melee.enums.Stage.BATTLEFIELD and custombot_state.position.y < 0
