...
class Retreat(Tactic):
    def shouldretreat(custombot_state, opponent_state, gamestate, camp):
        if custombot_state.invulnerability_left > 1:
            return False

        shieldactions = [Action.SHIELD_START, Action.SHIELD, Action.SHIELD_RELEASE, \
            Action.SHIELD_STUN, Action.SHIELD_REFLECT]

        losing = custombot_state.stock < opponent_state.stock
        if custombot_state.stock == opponent_state.stock:
            losing = custombot_state.percent > opponent_state.percent
        ...
        if opponent_state.action in [Action.DAIR_LANDING, Action.NAIR_LANDING, Action.FAIR_LANDING, \
                Action.UAIR_LANDING, Action.BAIR_LANDING, Action.LANDING] and custombot_state.action in shieldactions:
            return True

        # If opponent is falling, and we're in shield, retreat
        if opponent_state.speed_y_self < 0 and not opponent_state.on_ground and custombot_state.action in shieldactions:
            return True
        ...
        for projectile in gamestate.projectiles:
            if projectile.type in [melee.ProjectileType.SAMUS_BOMB, melee.ProjectileType.PIKACHU_THUNDER]:
                if custombot_state.position.x < projectile.x < opponent_state.position.x or custombot_state.position.x > projectile.x > opponent_state.position.x:
                    return True
    ...
    def step(self, gamestate, custombot_state, opponent_state):
        ...
        needswavedash = custombot_state.action in [Action.DOWN_B_GROUND, Action.DOWN_B_STUN, \
        ...
        """ Fox/Falco laser. Will need to the other projectile options """
        # Only do this laser if we are on the same level as the opponent
        if abs(opponent_state.position.y - custombot_state.position.y) < 5 and not missle_approaching:
        ...
        if (side_plat_height is not None) and (opponent_state.position.y+1 > side_plat_height) and (side_plat_left < opponent_state.position.x < side_plat_right):
            ...
            if custombot_state.position.y > 5 and custombot_state.on_ground:
                # Make sure we're in the center-ish of the platform
                if (other_side_plat_left+5 < custombot_state.position.x < other_side_plat_right-5) and not missle_approaching:
                    self.pickchain(Chains.Laser)
                    return
                else:
                    self.chain = None
